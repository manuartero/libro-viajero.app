...
Do not add more docs: types are the source of truth for shapes, and a design for unwritten code lives in its repo issue. `CHANGELOG.md` is generated by the release workflow, not a doc: past 0.1.0 (written by hand before the workflow existed), never edit it by hand.
