...
const shiftSuggestions = (project: Project) => {
  const returned = project.currentAssignments.filter((a) => a.returnedOn);
...
export function NextWeekPanel({ project }: NextWeekPanelProps) {
  const titleId = useId();

  const suggestions = shiftSuggestions(project);
